...
(async()=>{
  let checks=0;
  for(const file of ['poe2forge-planner.js','poe2forge-planner-data.json']) {
    const r=await request('/'+file);assert.equal(r.status,200);assert.equal(r.body,fs.readFileSync(path.join(root,file),'utf8'));assert.match(r.headers['content-type'],file.endsWith('.js')?/javascript/:/json/);checks++;
  }
  for(const file of ['poe2forge-proxy.js','tests/game-planner.cjs','tools/build-planner-catalog.cjs','another.json','poe2forge-planner-data.json.bak']) {assert.equal((await request('/'+file)).status,404);checks++;}
