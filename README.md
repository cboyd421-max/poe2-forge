...
- **Build-aware crossbow targets** — ask **Optimize my crossbow for DPS**, or **Change crossbow to include optimal DPS stats for level 62 character** for a matching imported level. Forge tests stat combinations with the actual selected skill, shows the best eligible improvement found, and previews a complete replacement item before applying it. Complete crossbow item text can also be pasted into the request field with Weapon selected.
...
- **Broader build-aware planning** — extend the first same-base crossbow search to other equipment, compare user-supplied candidates, support explicit budget and sustain constraints when evidence is available, and broaden natural-language intent handling. Recommendations must remain grounded in the imported build and measured PoB2 results, with reviewable item changes and undo.
...
This plain-language flow uses a local command parser with two modes: specific numeric equipment edits and build-aware crossbow DPS optimization. Shorthand such as **Change crossbow +2 to +4 proj skills** works with or without **from**, regardless of case. Specific edits require one existing modifier with one numeric value; ambiguous rings, duplicated matches, mismatched values and compound requests are rejected. Optimization requests use the imported character level and selected skill; a conflicting level or unsupported budget/objective clause is rejected. The parser does not call an AI provider or invent damage estimates. Every proposed item passes the complete-source equipment export before application.

**Optimize a crossbow.** Import the complete build, select a damaging skill under **Compare this skill**, then ask **Optimize my crossbow for DPS**. Local Forge searches the equipped rare crossbow's normal affix pool from the installed PoB2 data. It retains the base, item level, quality, sockets and rune/implicit context; replaces the explicit modifiers as a theoretical new-item target; and uses maximum rolls of the highest eligible tier per affix group. Two greedy passes consider prefixes/suffixes in different orders, enforcing up to three of each and one mod per group, with a cap of 260 distinct calculations. PoB2's native item comparison evaluates each combination against the whole imported build. This is the best improvement found within that search, not a global optimum or a crafting recipe.

The search filters affixes by the current item's level, checks the proposed item's required character level and weapon attributes using PoB2's data, and rejects increased attribute shortfalls elsewhere in the build. **Keep current life, energy shield, effective hit pool and resistances** is selected by default. This protects the listed numerical defences, not every utility, recovery, mana-sustain or gameplay property: review the complete before/after modifier list. Corrupted, mirrored, sanctified, unique and non-crossbow targets are outside this initial search. Prices, availability, a budget, other bases, other weapon sets, and broad multi-item or whole-build optimization are not evaluated.

Recommendations show the calculation count, skill, engine version, DPS/EHP comparison, base, item level and required level. **Apply & calculate** adopts the reviewed item and runs the normal complete-build calculation again; **Undo last change** restores the prior item and recalculates. Request edits, constraint changes, cancellation, mismatched result hashes and stale comparisons cannot apply an old recommendation. A complete PoB2 crossbow item can also be pasted into the request box with **Weapon** selected; it receives a full item preview and follows the same export/apply calculation path. Pasted item text is limited to 30 KB and is not automatically treated as a verified market listing or legal affix combination.
...
The in-memory cache holds at most 16 results and is keyed by XML, selected skill and the installed manifest identity. `POST /pob2-optimize` shares the loopback/Origin gate, request limits and single-job lock; its cache key also includes level/defence constraints and its mode. Normal calculations time out at 20 seconds per worker and optimization at 90 seconds. Cancellation terminates stale workers, and the browser checks response input hashes and its current comparison before accepting results. An installed-engine update invalidates old cache keys. GitHub Pages cannot launch a local process: automatic calculations and optimization require local Forge; the public site retains manual tools. The optional calculator currently supports Windows with a standard 64-bit Python 3 install.
...
**Testing optimization.** Workshop browser coverage includes the exact broad crossbow request, review-before-apply, complete item ingestion/export, automatic calculation/undo, level/hash/defence mismatches, unsupported constraints, and cancellation/stale replies. HTTP tests cover optimization gates, options and cache isolation. These use synthetic fixtures. Separately, installed PoB2 0.23.1 tested 119 combinations for the saved level 62 Mercenary and recommended a same-base Twin Crossbow target at about 24,678.5 Explosive Grenade DPS versus 14,941.1. Applying the generated complete item and recalculating reproduced the recommendation, with life 1,467, ES 201 and EHP about 2,178.2. Undo restored the prior candidate. This native example is a theoretical item comparison, not an observed price, listing, or in-game acquisition.
...
**Workshop trade matching.** Under Describe an upgrade, Find it on trade searches either the crossbow preview or the current candidate. Search target stats uses the same base and all supported explicit modifier values as minimums. Search close matches accepts any rare crossbow, keeps the target's projectile skill levels, and requires at least 60% of the other supported modifiers at 80% of their values. Both preserve the character-level cap, include Instant Buyout and In Person priced listings, and sort by price. League is visible and editable, with Forbidden Rites as the fallback when no saved or imported league is present; an optional maximum price supports Exalted, Divine, or Chaos. Preferences are saved separately from the build. No Forge OAuth, local server or listing API request is needed for these links. The official site may require its own sign-in.

The first mapping covers 17 common crossbow modifiers, with IDs checked against installed PoB2 0.23.1. Added damage compares the average of its endpoints. Unsupported lines are disclosed and omitted; implicit, rune and enchant effects are not treated as explicit stats. Searches do not guarantee availability, attribute eligibility, equal DPS, or the closest item by calculated DPS. To verify an actual listing, copy it into PoB2's item editor, then paste its complete PoB2 item text into Workshop with Weapon selected. Raw trade clipboard format is not yet a supported Workshop input. `node tests/trade.cjs`, `node tests/workshop.cjs`, and `node tests/planner-static.cjs` cover query construction, the browser workflow and static serving without upstream traffic.
...
- `poe2forge-trade.js` — client-side target/close-match crossbow search links to the official trade UI; no listing fetches or account access
